...
def a_star_quadtree(start_m, goal_m, qtm, movement='8n', occupancy_cost_factor=3):
    ...
    start_node_estimated_cost_to_goal = quadtreemap.Point.disOf2Points(quadtreemap.Point(start_m[0], start_m[1]), quadtreemap.Point(goal_m[0], goal_m[1]))
    candidates.put((start_node_estimated_cost_to_goal, 0, None, start))   # store (estimation, distance, previous-tile, current-tile)

    while candidates:
        est, dis, prev_node, curr_node = candidates.get()
        ...
        for til, deltacost in movements:
            ...
            if til not in path_record:
                ...
                candidates.put((dis + deltacost + potential_function_cost +quadtreemap.Point.disOf2Points(til.getCenter(), goal.getCenter()) + potential_function_cost, dis + deltacost + potential_function_cost, curr_node, til))
